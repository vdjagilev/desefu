...
class Kernel:
    ...
    def start(_version, _choices):
        ...
        parser.add_option("-l", "--log", action="store_true", default=False, help="Enable logging, which prints additional information the program is working right now")
        parser.add_option("-q", "--quiet", action="store_true", default=False, help="Disable any output, except formatted one")
        parser.add_option("--format", default=None, choices=_choices, help="Result is being shown in specific format %s" % _choices)
        parser.add_option("--date-format", default=None, help="Date format which is used for all output which is made to console, all required symbols can be found at https://docs.python.org/3.5/library/time.html#time.strftime. \nBy default locale appropriate time representation is done.")
